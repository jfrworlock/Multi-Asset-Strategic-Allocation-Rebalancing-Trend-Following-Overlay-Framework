...
Strategic allocation — the long-run GBP and percentage allocation assigned to each asset class.
Trend-following overlay — determines whether that strategic allocation is currently active, being accumulated, or temporarily excluded.
Execution engine — compares current exposure with active target exposure and generates either monthly sell-only rebalancing instructions or intra-month buy-only DCA (Dollar-Cost Averaging) instructions.
Settings:
